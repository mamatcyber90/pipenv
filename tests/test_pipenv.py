...
def create_empty_pipfile():
    if os.name == 'nt':
        return delegator.run('copy /y nul Pipfile').return_code
    else:
        return delegator.run('touch Pipfile').return_code

def get_pipfile_output(lock=False):
    if os.name == 'nt':
        if lock:
            return delegator.run('type Pipfile.lock').out
        else:
            return delegator.run('type Pipfile').out
    else:
        if lock:
            return delegator.run('cat Pipfile.lock').out
        else:
            return delegator.run('cat Pipfile').out
...
class TestPipenv():
    ...
    def test_cli_usage(self):
        ...
        lockfile_output = get_pipfile_output(lock=True)
